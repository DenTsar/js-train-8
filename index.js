...
function getUsersFromDatabase(array) {
  ...
  return Array.from(array, function (obj) {
    return {
      id: obj.id,
      firstName: obj.name.toUpperCase(),
      years: obj.age,
    };
  });
}
...
function getElementAtIndex(arr, index) {
  ...
  if (arr.length == 0 || index > arr.length) return "undefined";
...
function combineAndReverseArrays(arr1, arr2) {
  ...
  else return arr1.concat(arr2).reverse();
}
...
function findElementIndexes(arr, element) {
  ...
  if (!Array.isArray(arr)) return [];
    ...
    let indeks1 = arr.indexOf(element);
    let indeks2 = arr.lastIndexOf(element);
    return Array.of(indeks1, indeks2);
  }
}
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  if (
    !(arr.length > target - 1 && arr.length > start - 1 && arr.length > end - 1)
  )
...
function sortByKey(arr, key) {
  ...
  const fnSort = (a, b) => (a[key] < b[key] ? -1 : a[key] > b[key] ? 1 : 0);
...
function customUnshift(arr, ...elements) {
  ...
  for (let i = elements.length - 1; i >= 0; i--) arr.unshift(elements[i]);
...
function customAt(arr, index) {
  ...
  if (!Array.isArray(arr) || index > arr.length - 1) return undefined;
  const item = arr.at(index);
  if (typeof item == "number") console.log("Елемент є числом");
  if (typeof item == "string") console.log("Елемент є рядком");
  if (typeof item == "object") console.log("Елемент є обєктом");
  return item;
}
...
function customIncludes(arr, element) {
  ...
  console.log(arr.filter((a) => a == element).length);
